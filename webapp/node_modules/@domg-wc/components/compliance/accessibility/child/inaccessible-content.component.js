...
export const inaccessibleContent = ({ compliance, evaluation, limitations }) => {
    const inaccessibleContentTemplate = () => {
        ...
        return html `
            ...
            html `<h3>Niet-naleving van het bestuursdecreet</h3>
                    <ul>
                        ${limitations.withTiming.map((limitation) => html ` <li><p>${limitation}</p></li>`)}
                    </ul>`}
                ${limitations &&
            limitations.withoutTiming &&
            html `<h3>Onevenredige last</h3>
                    <ul>
                        ${limitations.withoutTiming.map((limitation) => html ` <li><p>${limitation}</p></li>`)}
                    </ul>`}
                ${limitations &&
            limitations.outsideApplicableLaw &&
            html `<h3>De inhoud valt buiten de werkingssfeer van de toepasselijke wetgeving</h3>
                    <ul>
                        ${limitations.outsideApplicableLaw.map((limitation) => html ` <li><p>${limitation}</p></li>`)}
                    </ul>`}
            </vl-typography>
        `;
    };
...
//# sourceMappingURL=inaccessible-content.component.js.map