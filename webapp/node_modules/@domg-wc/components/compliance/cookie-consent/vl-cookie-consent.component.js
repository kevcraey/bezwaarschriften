...
let VlCookieConsent = VlCookieConsent_1 = class VlCookieConsent extends BaseHTMLElement {
    constructor() {
        const html = `
    ...
    get _isAutoOpenDisabled() {
        return this.getAttribute(VlCookieConsent_1.attributePrefix + 'auto-open-disabled') != undefined;
    }
    get _isFunctionalOptInDisabled() {
        return this.getAttribute(VlCookieConsent_1.attributePrefix + 'auto-opt-in-functional-disabled') != undefined;
    }
    get _cookiePrefix() {
    ...
    _getOptInTemplate(optIn) {
        let template;
        if (optIn) {
            const checked = optIn.value || optIn.mandatory ? `${VlCookieConsent_1.attributePrefix}checked` : '';
            const mandatory = optIn.mandatory ? `${VlCookieConsent_1.attributePrefix}mandatory` : '';
            template = this._template(`
        <div class="vl-column vl-column--12">
          <vl-cookie-consent-opt-in ${VlCookieConsent_1.attributePrefix}label="${optIn.label}" ${VlCookieConsent_1.attributePrefix}description="${optIn.description}" ${checked} ${mandatory}></vl-cookie-consent-opt-in>
        </div>
      `);
    ...
    _resetOptInValue(optIn) {
        ...
        if (match) {
            optIn.value = optIn.getAttribute(VlCookieConsent_1.attributePrefix + 'checked') != undefined;
        }
    }
    _processOptIns() {
        this._optInElementen.forEach((optIn) => {
            this._processOptIn({
                name: optIn.id,
                label: optIn.getAttribute(VlCookieConsent_1.attributePrefix + 'label'),
                description: optIn.getAttribute(VlCookieConsent_1.attributePrefix + 'description'),
                value: optIn.getAttribute(VlCookieConsent_1.attributePrefix + 'checked') != undefined,
                mandatory: optIn.getAttribute(VlCookieConsent_1.attributePrefix + 'mandatory') != undefined,
            });
        });
    }
    _processOptIn({ name, label, description, value, mandatory, callback: { activated = null, deactivated = null } = {}, }) {
    ...
    _isCookieConsentCookieGeldig() {
        return (!isNaN(this._getCookieConsentDateCookie()) &&
...
VlCookieConsent = VlCookieConsent_1 = __decorate([
    webComponentConditional('vl-modal', 'vl-cookie-consent'),
    __metadata("design:paramtypes", [])
], VlCookieConsent);
export { VlCookieConsent };
//# sourceMappingURL=vl-cookie-consent.component.js.map