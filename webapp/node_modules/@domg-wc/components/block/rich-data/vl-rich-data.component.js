...
let VlRichData = class VlRichData extends BaseHTMLElement {
    constructor(content = '') {
        const html = `
        ...
        this.__onFilterFieldChanged = (event) => {
    ...
    __setHiddenInModalElements(hidden) {
        this.__searchFilter?.querySelectorAll('[hidden-in-modal]').forEach((element) => (element.hidden = hidden));
    }
    __observePager() {
        if (this.__pager) {
    ...
    __updateNumberOfSearchResults(number) {
        if (number) {
            if (this.__numberOfSearchResults)
                this.__numberOfSearchResults.textContent = String(number);
        }
        else {
            if (this.__pager) {
    ...
    __addSearchFilterEventListeners() {
        ...
        if (this.__searchFilterForm) {
            ...
            this.__searchFilterForm.addEventListener('keyup', ({ code }) => {
...
VlRichData = __decorate([
    webComponent('vl-rich-data'),
    __metadata("design:paramtypes", [Object])
], VlRichData);
export { VlRichData };
//# sourceMappingURL=vl-rich-data.component.js.map