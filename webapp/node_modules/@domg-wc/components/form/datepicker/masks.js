...
export const createDateMask = (format, minDate, maxDate) => {
    if (!format)
        return null;
    ...
    if (datePattern.filter((char) => !['d', 'm', 'Y', 'y'].includes(char)).length)
        return null;
...
export const createTimeMask = (format) => {
    if (!format)
        return null;
    ...
    // indien pattern niet ondersteund wordt, genereren we geen mask
    if (timePattern.filter((char) => !['h', 'm', 's'].includes(char)).length)
        return null;
    return {
        time: true,
