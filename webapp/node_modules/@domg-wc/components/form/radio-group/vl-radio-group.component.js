...
let VlRadioGroupComponent = class VlRadioGroupComponent extends FormControl {
    constructor() {
        super(...arguments);
        // Attributes
        this.readonly = radioGroupDefaults.readonly;
        this.value = radioGroupDefaults.value;
        // Variables
        this.initialValue = null;
        this.preventDefaultAndFocus = (event) => {
        ...
        this.handleKeyDown = (event) => {
            ...
            if (event.code === 'Space') {
                ...
                if (focusedRadio && !focusedRadio.hasAttribute('checked'))
                    this.checkSelectedRadio(focusedRadio);
            }
        };
    }
    static get styles() {
    ...
    preventDefaultEventOnAllInputsOfAllRadio(readonly) {
        this.getRadios()?.forEach((radio) => {
            if (readonly) {
    ...
    updateRadiosForAttribute(attribute) {
        const attributeKey = attribute;
        this.getRadios()?.forEach((radio) => this[attributeKey] ? radio.setAttribute(attribute, '') : radio.removeAttribute(attribute));
    }
    getRadios() {
        return Array.from(this.querySelectorAll('vl-radio'));
    }
    checkSelectedRadio(selectedRadio) {
        this.getRadios()?.forEach((radio) => {
    ...
    navigateRadioButtons(direction) {
        ...
        if (direction === 'ArrowLeft' || direction === 'ArrowUp') {
            ...
            newIndex = currentIndex + 1;
            if (newIndex >= radios.length)
                newIndex = 0;
        }
        this.checkSelectedRadio(radios[newIndex]);
        radios[newIndex].focus();
    }
    updateGroupAfterCheck(event) {
        const targetElement = event.target;
        this.checkSelectedRadio(targetElement);
    }
};
VlRadioGroupComponent = __decorate([
    webComponent('vl-radio-group')
], VlRadioGroupComponent);
export { VlRadioGroupComponent };
//# sourceMappingURL=vl-radio-group.component.js.map