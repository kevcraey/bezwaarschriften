 ...
 * @param webComponents
 */
export const registerWebComponents = (webComponents) => {
    if (FluxConfig.getPreferences().logTreeshakeRegistration) {
        console.debug(`treeshake registratie van`, webComponents.map((wc) => wc?.name).toString());
    }
};
/**
 ...
 * @param options
 */
export const defineWebComponent = (constructor, tagName, options) => {
...
export const debounce = (func, ms) => {
    let timer = undefined; // type van timer is number in oudere browsers, Timeout in nieuwste (wij kennen dat type niet)
    return (...args) => {
        clearTimeout(timer);
...
export const returnNotEmptyString = (s) => (s && s !== '' ? s : undefined);
export const returnNumber = (n) => (!isNaN(n) ? n : undefined);
export const ifDefinedString = (s) => ifDefined(returnNotEmptyString(s));
export const ifDefinedNumber = (n) => ifDefined(returnNumber(n));
/**
 ...
 */
export const findDeepestElementThroughShadowRoot = (rootElement, selector) => {
    if (rootElement) {
...
export const findNodesForSlot = (element, slotName) => {
    return element?.querySelectorAll(`:scope > [slot=${slotName}]`);
};
/**
 ...
 * @param slot
 */
export const isSlotEmpty = (slot) => slot
    .assignedNodes({ flatten: true })
    .every((node) => (node.nodeType === Node.TEXT_NODE && !node.textContent?.trim()) || node.nodeType === Node.COMMENT_NODE);
