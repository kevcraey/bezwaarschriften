...
export class VlCustomMap extends VlMapWithActions {
    ...
    getBaseLayers() {
        const layerCollection = this.getLayerGroup().getLayers();
        const firstLayer = layerCollection.getArray()[0];
        return firstLayer.getLayers().getArray();
    }
    getOverlayLayers() {
        return this._getOverlayLayersCollection().getArray();
    }
    _getOverlayLayersCollection() {
        const layerCollection = this.getLayerGroup().getLayers();
        const firstLayer = layerCollection.getArray()[1];
    ...
    initializeView(boundingBox, maxZoom) {
        const lambertExtent = transformExtent(getGRBLayerExtent(), getWGS84Code(), // bron = WGS84
