...
export class VlMapVectorLayer extends VlMapLayer {
    ...
    applyStyle(style) {
        if (style instanceof VlMapLayerStyle) {
            ...
            this._layer.setStyle((feature) => this._styles.map((style) => style.style(feature)).filter((style) => style != null));
        }
        else {
            this._styles = [];
            this._layer.setStyle(style);
        }
        this.dispatchEvent(new CustomEvent(VlMapVectorLayer.EVENTS.styleChanged, {
            bubbles: true,
            composed: true,
            detail: { style },
        }));
...
//# sourceMappingURL=vl-map-vector-layer.js.map